...
# --- View 2: Pareto Analysis ---
    ...
    if uploaded_file:
       ...
       fig.add_trace(
           go.Scatter(name='Cumulative_%', x=df2_sorted['area_name_en'], y=df2_sorted['Cumulative_%'], mode='lines+markers',
                      marker_color='red',
                      hovertemplate='<b>%{x}</b><br>Cumulative %: %{y:.2f}%<extra></extra>'),
           secondary_y=True,
       )

       # Configure x-axis and y-axes
       fig.update_xaxes(title_text='Area Name', tickangle=45)
       fig.update_yaxes(title_text='nRecords', secondary_y=False)
       fig.update_yaxes(title_text='Cumulative %', secondary_y=True)

       # y1-axis ticks at 0, 20k, ..., 100k
       ...
       # Add vertical lines at specific area names
